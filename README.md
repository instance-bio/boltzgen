> [!NOTE]
> This is the instance fork of /HannesStark/boltzgen

# Fork Notes

## Patch management

- `main` mirrors upstream 1-to-1.
- `dev` contains a patch-stack not in upstream. This is the branch you want if you want to build on top of the changes in this repo.

## Differences

- Uses (`uv`)[https://docs.astral.sh/uv/] as the depency management and build tool.
- Installable on non-linux non-CUDA hosts. CUDA and custom CUDA kernels like cuequivariance (eg used to accelerate triangle-attention) are optionally required via platform markers. This allows some amount of development on non-CUDA hosts. Actual deployments still require a linux + CUDA host.
...
```bash
uvx run git+https://github.com/instance/boltzgen.git
```

<details>
  <summary style="font-size: 1.3em; font-weight: 600;">
    Click for development installation with uv (Recommended)
  </summary>
...
After installation, **restart your terminal** or run `source $HOME/.local/bin/env` (macOS/Linux) to ensure `uv` is in your PATH.
...
This creates a `.venv` virtual environment and installs BoltzGen with all dependencies including development tools.

> **Note:** On macOS, only CPU-only PyTorch will be installed. On Linux with NVIDIA GPUs, CUDA-enabled PyTorch and cuEquivariance kernels will be installed automatically.

### 4 - Running BoltzGen

```bash
uv run boltzgen --help
```
...
```bash
uv run boltzgen run example/vanilla_protein/1g13prot.yaml \
...
2. Check whether your design specification is as intended.
    1. Run `uv run boltzgen check example/vanilla_peptide_with_target_binding_site/beetletert.yaml`.
...
3. Run the `uv run boltzgen run ...` command as above on your `.yaml` file.
...
5. You likely want to rerun the filtering step with different settings (takes ~15 sec). Use
   `uv run boltzgen run --steps filtering --output ...` or the Jupyter notebook `filter.ipynb` which is often more convenient.
...
- `intermediate_designs_inverse_folded/`: output of inverse folding, folding, and analysis steps
    - `/*.cif` and `/*.npz` : CIF and NPZ for designed proteins and targets after inverse folding. _Note: For designed residues, only the backbone atoms will have coordinates (sidechain coordinates will be 0,0,0)._
...
```bash
uv run boltzgen run example/cyclotide/3ivq.yaml \
...
```bash
uv run boltzgen run example/inverse_folding/1brs.yaml \
...
```bash
uv run boltzgen run example/binding_disordered_peptides/tpp4.yaml \
...
```bash
uv run boltzgen download all # downloads all models
uv run boltzgen download inverse-fold # downloads only the inverse folding model
```
...
```bash
uv run boltzgen configure example/cyclotide/3ivq.yaml \
...
This creates configuration files in `workbench/test-peptide-protein/` without running the actual design pipeline. You can edit these files if needed and then run `uv run boltzgen execute workbench/test-peptide-protein` to run the workflow.
...
```bash
uv run boltzgen merge workbench/run_a workbench/run_b workbench/run_c \
...
- `--output OUTPUT` – Destination directory for the merged data. The command creates (or replaces) the design artifacts inside this folder so that `uv run boltzgen run --steps filtering --output OUTPUT ...` can be executed afterwards.
